...
public class Menu {
    ...
    public void addButton(float offset, int column, int row, Texture texture, int scale) {
        float bx = pos.x + (offset + ((column + 1) * offset) + ( column * texture.getWidth())) * 2;
        float by = pos.y + (offset + ((row + 1) * offset) + ( row * texture.getHeight())) * 2;
    ...
    public void changeButton(int posId, float offset, int column, int row, Texture New_Texture, int scale) {
        buttons.remove(buttons.get(posId));
        addButton(offset, column, row, New_Texture, scale);
    }

    public void removeButton(int posId) {
        buttons.remove(buttons.get(posId));
    }
